...
function toNumber(value, def) {
  const suspect = Number(value);
  return Number.isNaN(suspect) ? def : suspect;
}
...
function getSumOfDigits(num) {
  // let result = 0;
  // const source = num.toString();
  // for (let i = 0; i < source.length; i += 1) result += Number(source[i]);
  // return result;
  return num
    .toString()
    .split('')
    .reduce((sum, digit) => sum + parseInt(digit, 36), 0);
}
...
function getCountOfOddNumbers(number) {
  ...
  // return isEven(abs) ? abs / 2 : (abs + 1) / 2;
